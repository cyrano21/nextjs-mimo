...
const lesson1 = {
  ...
  content: `
    <h2>Introduction aux Hooks Avancés</h2>
    <p>Dans cette leçon, nous allons explorer trois hooks React puissants qui vous aideront à gérer des états complexes et à optimiser les performances de vos applications : <code>useReducer</code>, <code>useCallback</code>, et <code>useMemo</code>.</p>
    
    <h3>useReducer</h3>
    <p><code>useReducer</code> est une alternative à <code>useState</code> pour gérer des logiques d'état plus complexes. Il est particulièrement utile lorsque l'état a plusieurs sous-valeurs ou lorsque le prochain état dépend du précédent de manière complexe. Il suit un modèle similaire à Redux avec un <em>reducer</em> et des <em>actions</em>.</p>
    
    <pre><code>// Signature de useReducer
const [state, dispatch] = useReducer(reducer, initialState, initFunction?);

// reducer: (state, action) => newState
// initialState: la valeur initiale de l'état
// initFunction (optionnel): une fonction pour calculer l'état initial de manière paresseuse</code></pre>

    <h3>useCallback</h3>
    <p><code>useCallback</code> est un hook qui mémorise une fonction. Cela signifie qu'il retourne une version mémorisée de la fonction callback qui ne change que si l'une de ses dépendances a changé. C'est particulièrement utile pour optimiser les composants enfants qui reçoivent des callbacks en props, car cela évite des re-renderings inutiles si la fonction n'a pas réellement changé (prévention des problèmes de référentielle égalité).</p>
    
    <pre><code>// Signature de useCallback
const memoizedCallback = useCallback(
  ...
  [a, b], // Tableau de dépendances
    ...
    <h3>useMemo</h3>
    <p><code>useMemo</code> est similaire à <code>useCallback</code>, mais au lieu de mémoriser une fonction, il mémorise une <em>valeur calculée</em>. Il exécute une fonction et mémorise son résultat. Le recalcul n'est effectué que si l'une des dépendances a changé. C'est utile pour les calculs coûteux en ressources afin d'éviter de les refaire à chaque rendu.</p>
    
    <pre><code>// Signature de useMemo
