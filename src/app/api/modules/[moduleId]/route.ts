...
interface ModuleLessonInfo {
  id: string; // Changed from number to string to match full lesson ID like "5-1"
  title: string;
  duration?: number;
  completed: boolean;
}
...
interface ModuleData {
  ...
  description: string;
  level: string;
  lessons: ModuleLessonInfo[]; // Now contains lessons with id: string
  duration: string;
...
export async function GET(
  ...
  const moduleData: ModuleData = {
    ...
    lessons: moduleLessons.map((lesson: Lesson): ModuleLessonInfo => {
      return {
        id: lesson.id, // Keep the full lesson ID like "5-1"
        title: lesson.title,
  ...
  console.log(`[API] Module ${moduleNumber} - Données finales:`, {
    id: moduleData.id,
    title: moduleData.title,
    lessonsCount: moduleData.lessons.length,
    lessonIds: moduleData.lessons.map(l => l.id)
  });
